...
/// Windows: query memory via GlobalMemoryStatusEx
...
fn query_vm_stats() -> Option<MemorySnapshot> {
    ...
    unsafe {
        ...
        if GlobalMemoryStatusEx(&mut status) == 0 {
            debug!("GlobalMemoryStatusEx failed");
            return None;
        }
        ...
        // Windows doesn't expose the same page-level breakdown as macOS.
        // We map available physical memory to the "free + inactive" concept.
        ...
        Some(MemorySnapshot {
            ...
            active_bytes: used,      // approximate: all non-free = "active"
            inactive_bytes: 0,       // Windows doesn't separate inactive pages
            wired_bytes: 0,          // Windows doesn't expose wired count
            compressed_bytes: 0,     // Windows compresses transparently
            available_bytes: available,
            pressure,
            timestamp: Instant::now(),
        })
...
/// Fallback for other platforms (Linux, etc.): always reports Normal
