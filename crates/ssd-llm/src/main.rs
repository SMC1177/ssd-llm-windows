...
fn main() -> Result<()> {
    ...
    match cli.command {
        Commands::Info { model } => {
            ...
            // Print quant type distribution
            let mut type_counts: std::collections::HashMap<String, usize> = std::collections::HashMap::new();
            for t in &gguf.tensors {
                *type_counts.entry(format!("{:?}", t.dtype)).or_insert(0) += 1;
            }
            ...
            // Show Q6K tensor names specifically
            let q6k_tensors: Vec<_> = gguf.tensors.iter().filter(|t| format!("{:?}", t.dtype) == "Q6K").collect();
            if !q6k_tensors.is_empty() {
                println!("  Q6K tensors:");
                for t in &q6k_tensors {
