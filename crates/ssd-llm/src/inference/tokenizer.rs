...
pub struct SimpleTokenizer {
    ...
    has_merges: bool,
    /// Tokenizer model type: "gpt2" or "llama" (SentencePiece)
    model_type: String,
    /// GPT-2 byte→unicode mapping (byte value → unicode char)
    byte_to_unicode: HashMap<u8, char>,
    /// GPT-2 unicode→byte mapping (unicode char → byte value)
...
fn build_gpt2_byte_unicode() -> (HashMap<u8, char>, HashMap<char, u8>) {
    ...
    for b in 0u16..=255 {
        let byte = b as u8;
        let ch = if (33..=126).contains(&b) || (161..=172).contains(&b) || (174..=255).contains(&b)
        {
            char::from(byte)
        } else {
...
impl SimpleTokenizer {
    pub fn from_gguf(gguf: &GgufFile) -> Self {
        ...
        let (byte_to_unicode, unicode_to_byte) = build_gpt2_byte_unicode();

        debug!(
            "Tokenizer: model={}, vocab={}, merges={}, add_bos={}",
            model_type,
            id_to_token.len(),
            merges.len(),
            add_bos
    ...
    pub fn encode(&self, text: &str) -> Vec<u32> {
        ...
        if self.model_type == "gpt2" && self.has_merges && !self.merge_ranks.is_empty() {
            // GPT-2 style BPE: byte-encode first, then apply merges
            tokens.extend(self.encode_gpt2_bpe(text));
        } else if self.has_merges && !self.merge_ranks.is_empty() {
    ...
    /// GPT-2 style BPE: convert bytes to unicode chars, then apply BPE merges
    fn encode_gpt2_bpe(&self, text: &str) -> Vec<u32> {
        // Convert each byte to its GPT-2 unicode representation
        let mut symbols: Vec<String> = Vec::new();
        for byte in text.bytes() {
            if let Some(&ch) = self.byte_to_unicode.get(&byte) {
                symbols.push(ch.to_string());
            }
        }

        // Iteratively apply the highest-priority merge (lowest rank)
        loop {
            if symbols.len() < 2 {
                break;
            }
        ...
        for sym in &symbols {
            if let Some(&id) = self.token_to_id.get(sym) {
                ...
                warn!("GPT-2 BPE: unknown symbol after merges: {:?}", sym);
            }
        }
    ...
    /// Decode a GPT-2 token string back to raw bytes
    fn decode_gpt2_token(&self, token: &str) -> String {
        let bytes: Vec<u8> = token
            .chars()
            .map(|ch| self.unicode_to_byte.get(&ch).copied().unwrap_or(b'?'))
            .collect();
        ...
        for &id in tokens {
            ...
            if let Some(token) = self.id_to_token.get(id as usize) {
                if self.model_type == "gpt2" {
                    text.push_str(&self.decode_gpt2_token(token));
                } else {
